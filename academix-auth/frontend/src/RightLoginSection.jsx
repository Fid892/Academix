import React from "react";
import { motion } from "framer-motion";

const RightLoginSection = () => {
  const handleGoogleLogin = () => {
    window.location.href = "http://localhost:5000/api/auth/google";
  };

  return (
    <motion.div
      initial={{ opacity: 0, x: 50 }}
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: 1, ease: "easeOut" }}
      className="login-container"
    >
      <motion.div
        whileHover={{ y: -5 }}
        className="premium-glass-card login-card-upgrade"
      >
        <div className="card-top-accent"></div>
        <div className="card-bottom-accent"></div>
        
        <div className="logo-box">
          <motion.div
            animate={{ scale: [1, 1.05, 1], rotate: [0, 5, -5, 0] }}
            transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
            className="logo-icon-v2"
          >
            A
          </motion.div>
          <h2>Welcome to Academix</h2>
          <p>Sign in to your account with Google</p>
        </div>

        <motion.button
          whileHover={{ scale: 1.02, boxShadow: "0 0 30px rgba(235, 64, 52, 0.4)" }}
          whileTap={{ scale: 0.98 }}
          onClick={handleGoogleLogin}
          className="google-btn-v2"
        >
          <svg viewBox="0 0 24 24" className="google-svg">
            <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.26.81-.58z" />
            <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
          Continue with Google
        </motion.button>
        
        <div className="card-footer">
          <span>By signing in, you agree to our Terms of Use and Privacy Policy.</span>
        </div>
      </motion.div>
    </motion.div>
  );
};

export default RightLoginSection;
