...
const FacultyFeedPage = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/announcements/faculty", { credentials: "include" });
      const data = await res.json();
  ...
  return (
    <div className="student-container">
      ...
      <div className="welcome-section" style={{ marginTop: "40px" }}>
        <h1 className="welcome-name">🎓 Faculty Announcements</h1>
        <p className="welcome-label" style={{ marginTop: "8px" }}>View official announcements from faculty and departments</p>
      </div>
